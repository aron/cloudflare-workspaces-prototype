...
describe("SQLiteWorkspaceProvider — writeSync", () => {
  ...
  it("extends the file when writing past EOF", () => {
    const p = makeProvider();
  ...
  it("zero-fills the gap when writing past current size", () => {
    const p = makeProvider();
  ...
  it("writes across a chunk boundary, splicing in the affected chunks only", () => {
    const p = makeProvider();
  ...
  it("openSync('a') starts the fd at EOF", () => {
    const p = makeProvider();
    p.writeFileSync("/a", "hello");
...
describe("SQLiteWorkspaceProvider — truncateSync / ftruncateSync", () => {
  ...
  it("truncateSync to 0 leaves an empty file", () => {
    const p = makeProvider();
    p.writeFileSync("/a", "hello");
    p.truncateSync("/a", 0);
    expect(p.statSync("/a").size).toBe(0);
  ...
  it("truncateSync at the same size is a no-op", () => {
    const p = makeProvider();
    p.writeFileSync("/a", "hello");
    p.truncateSync("/a", 5);
    expect(p.readFileSync("/a", "utf8")).toBe("hello");
