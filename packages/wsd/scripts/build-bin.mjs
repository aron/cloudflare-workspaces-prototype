#!/usr/bin/env node
// Cross-compile wsd to a self-contained Node SEA binary for each supported
...
import { bundleWsd } from "./sea/bundle.mjs";
...
const seaWorkDir = resolve(wsdRoot, "dist/sea");
...
async function main() {
  ...
  for (const target of targets) {
    console.log(`[wsd-bin] building ${target.name}`);
...
async function buildTarget(target) {
  ...
  await bundleWsd({ outfile: bundlePath, target });
...
async function ensureTargetNodeBinary(target) {
  ...
  if (!(await exists(archivePath))) {
    ...
    console.log(`[wsd-bin] downloading ${url}`);
...
async function maybe(promise) {
  try {
    ...
    console.warn(`[wsd-bin] non-fatal: ${err.message}`);
  }
}

function runNpm(script) {
  return new Promise((res, rej) => {
    const child = execFile("npm", ["run", script], { cwd: wsdRoot });
