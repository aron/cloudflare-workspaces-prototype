#!/usr/bin/env node
// Build the wsd-linux-x64 SEA binary, stage it into the platform
// package, and produce a docker image that downstream Dockerfiles
// can `COPY --from`. Tagged `cloudflare/workspace-wsd-linux-x64`
...
// when artifacts/wsd/wsd-linux-x64 is already current.
...
const wsdRoot = resolve(here, "..");
const repoRoot = resolve(wsdRoot, "../..");
const platformDir = resolve(repoRoot, "packages/wsd-linux-x64");

const { version } = JSON.parse(readFileSync(resolve(wsdRoot, "package.json"), "utf8"));

const IMAGE = "cloudflare/workspace-wsd-linux-x64";
const push = process.argv.includes("--push");

// 1. Build the SEA binary.
console.log("[build-docker] building wsd-linux-x64");
execFileSync("node", ["./scripts/build-bin.mjs"], { cwd: wsdRoot, stdio: "inherit" });

// 2. Stage the binary into the platform package's bin/. The
//    .dockerignore restricts the build context to bin/wsd, so the
//    Dockerfile's COPY hits this file directly.
const src = resolve(repoRoot, "artifacts/wsd/wsd-linux-x64");
const dst = resolve(platformDir, "bin/wsd");
