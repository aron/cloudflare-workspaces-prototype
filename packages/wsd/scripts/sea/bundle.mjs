// Bundle wsd into a single ESM file for Node's native SEA.
//
// Strategy:
//   * esbuild produces an ESM bundle of `dist/cli/wsd.cjs`. ESM is required so
...
export async function bundleWsd({ outfile, target }) {
...
function loadNative() {
	...
	const dir = join(tmpdir(), "wsd-sea-" + hash);
...
`;
  ...
  const nativeShimPlugin = {
    ...
    setup(b) {
      ...
      b.onLoad({ filter: /.*/, namespace: "wsd-shim" }, (args) => {
