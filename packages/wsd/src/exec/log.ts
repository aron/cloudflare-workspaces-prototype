...
// Create a new log row in wsd_exec_meta. Throws if a row with the
...
export function createLog(db: Database, id: string, opts: LogOptions): EventLog {
  ...
  db.run(
    `INSERT INTO wsd_exec_meta (exec_id, started_at, bytes, evicted) VALUES (?, ?, 0, 0)`,
    id,
    startedAt,
  );
...
export function openLog(db: Database, id: string, opts: LogOptions): EventLog | undefined {
  const meta = db.one<MetaRow>(`SELECT * FROM wsd_exec_meta WHERE exec_id = ?`, id);
...
export class EventLog {
  ...
  append(name: "stdout" | "stderr", value: Uint8Array): number {
    ...
    this.db.transactionSync(() => {
      this.db.run(
        `INSERT INTO wsd_exec_log (exec_id, seq, ts, kind, value) VALUES (?, ?, ?, ?, ?)`,
        this.id,
        seq,
        ts,
        kind,
      ...
      this.db.run(`UPDATE wsd_exec_meta SET bytes = ? WHERE exec_id = ?`, newBytes, this.id);
    });
    return seq;
  }
  ...
  setExit(code: number): number {
    ...
    this.db.transactionSync(() => {
      ...
      this.db.run(
        `UPDATE wsd_exec_meta SET exited_at = ?, exit_code = ? WHERE exec_id = ?`,
        ts,
        code,
        this.id,
      );
  ...
  dispose(): void {
    this.db.transactionSync(() => {
      this.db.run(`DELETE FROM wsd_exec_log WHERE exec_id = ?`, this.id);
      this.db.run(`DELETE FROM wsd_exec_meta WHERE exec_id = ?`, this.id);
    });
  }
  ...
  *replay(after: number | "tail" = 0): IterableIterator<ExecEvent> {
    ...
    const rows = this.db.all<EventRow>(
      `SELECT seq, kind, value FROM wsd_exec_log
  ...
  private meta(): MetaRow | undefined {
    return this.db.one<MetaRow>(`SELECT * FROM wsd_exec_meta WHERE exec_id = ?`, this.id);
  }

  private evict(): void {
    this.db.transactionSync(() => {
      this.db.run(`DELETE FROM wsd_exec_log WHERE exec_id = ?`, this.id);
      this.db.run(`UPDATE wsd_exec_meta SET bytes = 0, evicted = 1 WHERE exec_id = ?`, this.id);
    });
  }
}
