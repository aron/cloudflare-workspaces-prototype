// Regression: wsd must not deadlock when shell.exec passes a cwd
// that lives inside its own FUSE mount.
//
// The shape of the bug, which `runner.ts` now defends against:
//
//   1. wsd holds the FUSE session for MOUNT_POINT (e.g. /workspace).
...
//   3. If the child's chdir target is inside wsd's own FUSE mount,
//      the kernel issues a FUSE LOOKUP that needs to be answered
//      by wsd's event loop.
//   4. wsd's event loop is blocked in uv_spawn waiting for the
...
// chdir after the spawn dance has returned and wsd's event loop
// is responsive again.
//
// Running this test requires:
//   - Docker with /dev/fuse, --privileged, SYS_ADMIN (DinD or
//     native Linux). It boots the linux-x64 wsd SEA binary from
//     artifacts/wsd/ in a privileged container so the kernel FUSE
//     driver is actually in the loop.
//   - The wsd binary built and staged at the path
//     packages/workspace/test-harness/run-wsd.sh expects. The
...
const WSD_BINARY = join(REPO_ROOT, "artifacts/wsd/wsd-linux-x64");
const RUN_WSD_SCRIPT = join(REPO_ROOT, "packages/workspace/test-harness/run-wsd.sh");
...
describeIfReal("Runner shell.exec under real FUSE", () => {
  ...
  beforeAll(async () => {
    // Boot a real-FUSE wsd container via the existing harness
    // helper. The helper prints the URL on stdout and the
    // container id on stderr (WSD_HARNESS_CID=...).
    const proc = spawn("bash", [RUN_WSD_SCRIPT], {
      env: { ...process.env, WSD_HARNESS_PORT: "0" },
      stdio: ["ignore", "pipe", "pipe"],
    });
    if (!proc.stdout || !proc.stderr) {
      throw new Error("run-wsd.sh did not expose stdout/stderr pipes");
    }
    ...
    if (proc.exitCode !== 0) {
      throw new Error(`run-wsd.sh exited ${proc.exitCode}: ${stderr}`);
    }
    url = stdout.trim();
    const match = stderr.match(/WSD_HARNESS_CID=([0-9a-f]+)/);
  ...
  test("exec(cwd inside the FUSE mount) returns quickly and runs the command", async () => {
    if (!url) throw new Error("wsd container did not start");
    const client = createWorkspaceClient({
      url: `${url.replace(/^http(s?):\/\//, "ws$1://")}/ws`,
      WebSocketImpl: WebSocket,
    });
    try {
      // Pre-fix this hangs forever and wedges the wsd event loop.
