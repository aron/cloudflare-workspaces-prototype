...
function duplicate(path: string, idx: number, total: number, n: number): Error {
  if (total === 1) {
    return new Error(`Found ${n} occurrences of the text in ${path}. The text must be unique. Please provide more context to make it unique.`);
  }
  return new Error(`Found ${n} occurrences of edits[${idx}] in ${path}. Each oldText must be unique. Please provide more context to make it unique.`);
}
...
export function applyEditsToNormalizedContent(
  ...
  const initial = normalized.map(e => fuzzyFindText(normalizedContent, e.oldText));
