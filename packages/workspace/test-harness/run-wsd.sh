...
BINARY="${WSD_BINARY:-$ROOT/artifacts/wsd/wsd-linux-x64}"
HOST_PORT="${WSD_HARNESS_PORT:-0}"
...
if [[ ! -x "$BINARY" ]]; then
  echo "wsd binary not found at $BINARY" >&2
  echo "Run 'npm run build:bin --workspace @cloudflare/workspace-wsd' first" >&2
  exit 1
fi
...
# exists.
IMAGE_TAG="wsd-harness:libfuse2"
...
# Wait for /health. wsd takes a couple of seconds in the
...
echo "wsd did not become healthy at $URL" >&2
docker logs "$CID" >&2 || true
docker kill "$CID" >/dev/null 2>&1 || true
exit 1
