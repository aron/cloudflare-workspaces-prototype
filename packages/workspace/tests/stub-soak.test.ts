...
// reaches into the DO via the WSD binding, pulls a WorkspaceStub
...
import type { Env, WsdContainer } from "./stub-soak-worker.js";
...
async function runSoak(
  ...
  const id = env.WSD.idFromName(`soak-${Math.random()}`);
  const stub = env.WSD.get(id);
...
describe("WorkspaceStub disposal soak (DO ↔ Worker)", () => {
  ...
  it("snapshot reads do not themselves grow counters", async () => {
    ...
    const id = env.WSD.idFromName("snapshot-only");
    const stub = env.WSD.get(id);
  ...
  it("runInDurableObject reaches the same counters as the RPC path", async () => {
    ...
    const id = env.WSD.idFromName("direct-access");
    const stub = env.WSD.get(id);
    const snap = await runInDurableObject(stub, async (instance: WsdContainer) => {
