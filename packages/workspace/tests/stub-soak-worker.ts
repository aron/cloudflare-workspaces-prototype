...
// Two exports:
//
//   - WsdContainer (DO): owns a Workspace backed by an in-process
//     fake backend (no wsd, no network). Has two RPC methods:
...
//                            @cloudflare/workspace-rpc/debug.
//
...
// Fake backend so the Workspace doesn't try to dial wsd. Sync
...
export default class TestDriver extends WorkerEntrypoint<Env> {
  ...
  async runSoak(opts: {
    ...
    const id = this.env.WSD.idFromName("soak");
    const stub = this.env.WSD.get(id);
