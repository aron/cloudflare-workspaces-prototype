...
describe("App", () => {
  ...
  test("updates running elapsed time before agents finish", async () => {
    ...
    expect(within(screen.getByLabelText("Workspace runtime wing")).getByText("00:02")).toBeTruthy();
  ...
  test("renders telemetry and grouped activity for live runtime events", async () => {
    ...
    expect(within(workspace).getByText("Tool calls")).toBeTruthy();
    expect(within(workspace).getByText("1")).toBeTruthy();
    expect(within(workspace).getByText("Exec calls")).toBeTruthy();
    ...
    expect(within(sandbox).getAllByText("1").length).toBeGreaterThanOrEqual(2);
    expect(within(sandbox).getByText("awake")).toBeTruthy();
  ...
  test("renders completed run telemetry and capacity failure hints", async () => {
    ...
    expect(within(workspace).getByText("02:50")).toBeTruthy();
    expect(within(sandbox).getByText(/failed/)).toBeTruthy();
    expect(within(sandbox).getByText("03:40")).toBeTruthy();
