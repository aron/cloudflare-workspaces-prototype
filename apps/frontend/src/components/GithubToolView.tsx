/**
 * Specialized renderer for `tool-github` parts — the interactive GitHub OAuth
 * onboarding card. Structurally identical to `CloudflareToolView` (the `github`
 * tool is the same streaming tool over a different provider): on `connect` it
 * yields an interim `{ phase: "awaiting_auth", authUrl }` chunk and then stays
 * pending while the server polls for the connection to go READY.
 *
 *   - Continue → opens the OAuth authUrl in a new tab.
 *   - Cancel   → cancels the tool call (via `onCancel(toolCallId)`).
 *
 * When the poll resolves, the tool's final output replaces the chunk with a
 * terminal phase (`ready` / `cancelled` / `timeout` / `failed`).
 * `status`/`disconnect` produce a single terminal object and render the same.
 */
import {
  CheckCircle2,
  ExternalLink,
...
interface GithubToolViewProps {
  output?: GithubOutput | null;
  errorText?: string;
  state?: string;
  toolCallId?: string;
...
function resolve(
  output: GithubOutput | null | undefined,
...
export function GithubToolView({
  output,
  errorText,
  state,
  toolCallId,
  ...
  const icon =
    ...
    : status.kind === "await" || status.kind === "running"
      ...
      : <GitBranch className="size-4 text-kumo-inactive" />;
  ...
  return (
    <div className={cn("my-2 overflow-hidden rounded-lg border", chrome)}>
      <header className="flex items-center gap-2 border-b border-current/20 px-3 py-1.5">
        {icon}
        <GitBranch className="size-3.5 text-kumo-inactive" />
        <span className="text-xs font-semibold text-kumo-default">GitHub</span>
        <span
          className={cn(
            "text-xs",
        ...
        <div className="space-y-3 p-4">
          <div>
            <p className="text-sm font-medium text-kumo-default">
              {output?.title ?? "Authorize GitHub access"}
            </p>
            <p className="mt-1 text-sm text-kumo-subtle leading-snug">
              {output?.message ??
                "Open the authorization link to grant read/maintain access to your GitHub account. This window keeps waiting until you finish."}
            </p>
          </div>
