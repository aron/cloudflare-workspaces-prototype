 ...
 *   2. POST /api/app/rooms                     → create room (AppDO row)
 *   3. GET  /api/app/rooms                     → list contains the new room
 *   4. GET  /api/rooms/:id/meta                → RoomDO was initialized by worker
 ...
 *
 * Each step uses real bindings (RoomDO, AppDO, Agent DO, Sandbox container).
...
describe("E2E: identity + rooms", () => {
  ...
  it("worker initializes the RoomDO so /meta is immediately available", async () => {
...
describe("E2E: Agent DO seed", () => {
  it("RoomDO seeded the Agent DO with the originating user message", async () => {
    if (!threadId) throw new Error("threadId missing — prior test must have failed");

    // Poll: the seed call is awaited inline by RoomDO, but `saveMessages` may
