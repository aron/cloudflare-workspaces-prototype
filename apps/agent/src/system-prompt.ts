...
const GUIDELINES = [
  ...
  "exec has three backends. 'shell' (default): just-bash in an isolate, instant boot, built-in git — use for text plumbing (grep / find / sed) and git. 'javascript': evaluates an ES module in the same kind of cheap isolate — a lightweight alternative to 'container' that is faster and cheaper but needs real code; export a default function, pass a JSON `input`, get a JSON `result` back, set per-execution env vars with `env`, and use node:fs/promises for files and fetch() for network. 'container': full Node/Bun toolchain (bun, npm, node, tsc, wrangler, esbuild) — reach for it only when the work needs a real binary",
  ...
  "'javascript' backend shape: `command` is ES module source that must `export default`. Export a value or a function; a function receives the JSON `input` as its argument and its return value comes back as the JSON `result`. Read/write the workspace with node:fs/promises (paths are absolute under /workspace, or relative to `cwd`), and call fetch() for network. Example — read a file: `import { readFile } from 'node:fs/promises'; export default async () => await readFile('/workspace/hello.js', 'utf-8');`. Example — fetch + transform with input: `export default async ({ url }) => { const r = await fetch(url); return { status: r.status, body: await r.text() }; };` run with input `{ \"url\": \"https://example.com\" }`",
