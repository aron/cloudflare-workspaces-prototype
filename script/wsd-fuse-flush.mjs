...
//
// Boots one wsd container with FUSE mounted on /workspace, writes
...
//
// Knobs:
//   WSD_BINARY   path to wsd-linux-x64 (default artifacts/wsd/...)
...
const BINARY = process.env.WSD_BINARY ?? resolve(REPO_ROOT, "artifacts/wsd/wsd-linux-x64");
const KEEP = process.env.KEEP === "1";

if (!existsSync(BINARY)) {
  console.error(`wsd binary not found at ${BINARY}`);
  console.error("run `npm run build:bin --workspace @cloudflare/workspace-wsd` first");
  process.exit(2);
}
...
async function bootContainer() {
  ...
  args.push(
    ...
    // same mount namespace where wsd mounted FUSE. wsd backgrounds
    // and we tail /tmp/wsd.log for debugging.
    "apt-get update >/dev/null 2>&1 && " +
      "apt-get install -y --no-install-recommends fuse3 libfuse2t64 >/dev/null 2>&1 && " +
      "mkdir -p /workspace && " +
      "/usr/local/bin/wsd >/tmp/wsd.log 2>&1 & " +
      "WSD_PID=$!; " +
      "trap 'kill $WSD_PID 2>/dev/null' EXIT; " +
      "wait $WSD_PID",
  );
...
async function main() {
  process.stderr.write("booting wsd container ...\n");
  ...
  try {
    ...
    const { createWorkspaceClient } = await import(
      `${REPO_ROOT}/node_modules/@cloudflare/workspace-rpc/dist/client.js`
    );
    ...
    const { pullOnce } = await import(
      `${REPO_ROOT}/node_modules/@cloudflare/workspace-rpc/dist/sync-driver.js`
    );
    ...
    // Dump wsd's logs to help diagnose.
    try {
      const { stdout } = await dockerExec(container.cid, "cat", "/tmp/wsd.log");
      process.stderr.write(`\n--- wsd logs ---\n${stdout}\n----------------\n`);
    } catch {
